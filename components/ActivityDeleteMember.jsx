...
const ActivityDeleteMember = ({ data }) => {
  const dateTime = data.createAt.split(", ");
  let membersString = "";
  if (data.additionalInfo.deletedMembersName?.length > 1) {
    membersString = data.additionalInfo.deletedMembersName.join(", ");
  } else {
    membersString = data.additionalInfo.deletedMembersName;
  }
  return (
    <View
    ...
    >
      ...
      <View className="flex-col px-2 space-y-1" style={{ flex: 1 }}>
        <View className="flex-row items-center" style={{ flexWrap: "wrap" }}>
          ...
          <Text className="mr-1">have removed</Text>
          <Text
            className="text-gray-700 mr-1"
            style={{
              fontSize: 15,
          ...
          <Text className="mr-1">from the group</Text>
          <Text
...
export default ActivityDeleteMember;
